...
> 物理含义：HRS 电流与 h-BN 缺陷参与的场辅助跳跃/隧穿相容。提案里的 Poole–Frenkel 也能较好拟合；单温度、窄电压窗不能严格区分 hopping、TAT、PF 或 Schottky。**VULCAN-2D 采用有详细平衡依据的 sinh 有效输运律，但不把它唯一命名为 TAT。**
...
→ 低压 I≈(I_sat/V_k)·V，高压 I→I_sat(1+λV) 饱和。**1T1R 端口的 LRS 电流主要由晶体管负载线限制**，这解释了 R2（I_cc CV 仅 1%）。h-BN 内部的高传输态可以来自局部金属辅助通道或本征缺陷桥；只有观察到量子化电导或原子瓶颈证据后，才适合启用 QPC 模型。
...
| ② 电荷输运 | **HRS：有效缺陷辅助 sinh；LRS：由晶体管负载线限制，局部金属/缺陷桥机制待表征** |
| ③ 焦耳热/温度 | 单温度数据，未直接标定；可由 V_set 的弱正自相关与耐久漂移间接约束 |
| ④ 器件变异性 | I_cc CV=1% vs V_set/电阻 CV=24–46%；电阻对数正态、电压正态——**直接给出变异性注入的分布参数** |

**一句话结论**：这是一只双极性 h-BN 1T1R 器件——*h-BN 中受二维结构限制的局部缺陷区域在电场下逐步进入高传输态，晶体管通过负载线限制通道继续发展*；置位/复位电压各有约 25% 波动，而限流电流被钳到 1% 以内，存储窗口约 600 倍并随循环缓慢收窄。局部通道究竟由金属离子、硼空位层间桥还是二者共同形成，需要 CAFM、XTEM 和元素分析进一步判断。
